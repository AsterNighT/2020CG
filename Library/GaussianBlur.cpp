...
namespace Library
{
	...
	void GaussianBlur::InitializeSampleOffsets()
	{
		...
		for (UINT i = 0; i < sampleCount / 2; i++)
		{
			...
			mHorizontalSampleOffsets[i * 2 + 1] = vec2(horizontalOffset, 0.0f);
			mHorizontalSampleOffsets[i * 2 + 2] = vec2(-horizontalOffset, 0.0f);

			mVerticalSampleOffsets[i * 2 + 1] = vec2(0.0f, verticalOffset);
			mVerticalSampleOffsets[i * 2 + 2] = vec2(0.0f, -verticalOffset);
		}
	}
