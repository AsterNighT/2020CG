...
namespace Library
{
	...
	void Game::UnbindPixelShaderResources(UINT startSlot, UINT count)
	{
		...
		static std::vector<ID3D11ShaderResourceView*> emptySRV;
		emptySRV.resize(count, nullptr);

		mDirect3DDeviceContext->PSSetShaderResources(0, 3, &emptySRV.at(0));
	}
    ...
    void Game::InitializeDirectX()
    {
		...
		ImGui_ImplDX11_Init(mWindowHandle, direct3DDevice, direct3DDeviceContext);
    ...
    {
		ImGui_ImplDX11_WndProcHandler(windowHandle, message, wParam, lParam);
