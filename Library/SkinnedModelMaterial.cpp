...
namespace Library
{
    ...
    void SkinnedModelMaterial::CreateVertexBuffer(ID3D11Device* device, const Mesh& mesh, ID3D11Buffer** vertexBuffer) const
    {
		...
		for (UINT i = 0; i < sourceVertices.size(); i++)
		{
			...
			vec3 uv = textureCoordinates->at(i);
			...
			vertices.push_back(VertexSkinnedPositionTextureNormal(vec4(position.x, position.y, position.z, 1.0f), vec2(uv.x, uv.y), normal, XMUINT4(indices), vec4(weights)));
		}
